...
sys_exofork(void)
{
	...
	if (ret < 0)
		return (ret == -E_NO_FREE_ENV) ? -E_NO_FREE_ENV : -E_NO_MEM;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((uintptr_t)srcva >= UTOP || dstva != ROUNDDOWN(dstva, PGSIZE) || (uintptr_t)dstva >= UTOP || dstva != ROUNDDOWN(dstva, PGSIZE))
		return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uintptr_t)srcva < UTOP && (uintptr_t)dstenv->env_ipc_dstva < UTOP) {
		...
		pte_t *srcpte;
		struct PageInfo *pp;
		if (!(pp = page_lookup(curenv->env_pgdir, srcva, &srcpte)))
			return -E_INVAL;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
		...
		/* syscalls for environment information */
		case SYS_getenvid:
			return sys_getenvid();
		
		/* syscalls for environment lifetime management */
		case SYS_exofork:
			return sys_exofork();
		...
		/* syscalls for user page fault upcall */
